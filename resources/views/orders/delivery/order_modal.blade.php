            ...
            <div class="modal-body">
                <form id="deliveryOrderForm" action="{{ url('delivery/store') }}" enctype="multipart/form-data" method="post">
                    ...
                    <div class="row">
                        {{-- office and agent --}}
                        <div class="col-12">
                            @include('orders.delivery.office_agent')
                        </div>
                        {{-- property information --}}
                        <div class="col-12  mt-4">
                            @include('orders.delivery.property_information')
                        </div>
                        {{-- desired date --}}
                        <div class="col-12 col-md-6 col-lg-6 mt-4">
                            @include('orders.delivery.service_date')
                        </div>
                        {{-- signpost and  accessorries --}}
                        <div class="col-12   mt-4">
                            @include('orders.delivery.posts_signs_accessories')
                        </div>
                        {{-- Comment --}}
                        <div class="col-12   mt-4">
                            @include('orders.delivery.comment')
                        </div>
                        {{-- Footer --}}
                        <div class="col-12   mt-4">
                            @include('orders.delivery.footer')
                        </div>
                    </div>
                </form>
            </div>
