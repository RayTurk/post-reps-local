...
const agentAccountingPayments = {
    ...
    async datatablePayments() {
        ...
        agentAccountingPayments.table = $(tableId).dataTable({
            language: {
                search: "_INPUT_",
            ...
            pageLength: 10,
            dom: "rtip",
            ajax: helper.getSiteUrl("/datatable/agent/accounting/payments"),
            serverSide: true,
            columnDefs: [
    ...
    accountingPaymentsInput() {
        ...
        inputs.forEach(input => {
            ...
            input.on("keyup", (event) => {
                ...
                agentAccountingPayments.table.fnFilter(input.value);
            });
        })
    },
    ...
    getStatus(status, orderType) {
        ...
        if (status == agentAccountingPayments.status_received) {
            statusDescription = "Received";
        }

        if (status == agentAccountingPayments.status_incomplete) {
            statusDescription = "Action Needed";
        }

        if (status == agentAccountingPayments.status_cancelled) {
            statusDescription = "Cancelled";
        }

        if (status == agentAccountingPayments.status_scheduled) {
            statusDescription = "Scheduled";
        }

        if (status == agentAccountingPayments.status_completed) {
            if (orderType == 'install') {
...
$(() => {
    agentAccountingPayments.init();
});

export default agentAccountingPayments;
