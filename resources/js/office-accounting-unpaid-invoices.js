...
const officeAccountingUnpaidInvoices = {
    ...
    async datatableUpaidInvoices() {
        ...
        officeAccountingUnpaidInvoices.table = $(tableId).dataTable({
            language: {
                search: "_INPUT_",
            ...
            pageLength: 10,
            dom: "rtip",
            ajax: helper.getSiteUrl("/datatable/office/accounting/unpaid/invoices"),
            serverSide: true,
            columnDefs: [
                ...
                },
                {
                    ...
                    render(d, t, r) {
                        return `
                            ...
                            ${officeAccountingUnpaidInvoices.userRole == 1 ? '<a href="#" class="text-primary edit-invoice" data-invoice-id="'+r.id+'">EDIT</a>' : ''}
                        `;
                    }
                },
                {
    ...
    unpaidInvoicesInput() {
        ...
        inputs.forEach(input => {
            ...
            input.on("keyup", (event) => {
                ...
                officeAccountingUnpaidInvoices.table.fnFilter(input.value);
            });
        })
    },
    ...
    getStatus(status, orderType) {
        ...
        if (status == officeAccountingUnpaidInvoices.status_received) {
            statusDescription = "Received";
        }

        if (status == officeAccountingUnpaidInvoices.status_incomplete) {
            statusDescription = "Action Needed";
        }

        if (status == officeAccountingUnpaidInvoices.status_cancelled) {
            statusDescription = "Cancelled";
        }

        if (status == officeAccountingUnpaidInvoices.status_scheduled) {
            statusDescription = "Scheduled";
        }

        if (status == officeAccountingUnpaidInvoices.status_completed) {
            if (orderType == 'install') {
    ...
    processPayment() {
        $('body').on('click', '.process-payment', async (e) => {
            ...
            officeAccountingUnpaidInvoices.originalInvoiceAmount = invoiceAmount;

            if (invoiceId) {
                helper.showLoader();

                //Update invoice id in form
                officeAccountingUnpaidInvoices.form.find('[name="invoice_id"]').val(invoiceId);

                //Amount due
                officeAccountingUnpaidInvoices.form.find('[invoice-amount-due]').html(invoiceAmount);

                $.get(`${helper.getSiteUrl()}/accounting/unpaid/invoice/payer/${invoiceId}`)
                .done(officeUser => {
                    //console.log(user)
                    officeAccountingUnpaidInvoices.officeUser = officeUser;

                    //If office is invoiced then apply convenience fee
                    //Also add the fee to amount due
                    if (officeUser.payment_method == 2) {
                        officeAccountingUnpaidInvoices.form.find('#convenienceFeeMessageDiv').removeClass('d-none');
                    }

                    $(`[billing-name]`).val(officeUser.name);
                    $(`[billing-address]`).val(officeUser.address);
                    $(`[billing-city]`).val(officeUser.city);
                    $(`[billing-state]`).val(officeUser.state);
                    $(`[billing-zip]`).val(officeUser.zipcode);

                    if (officeUser.authorizenet_profile_id) {
                        officeAccountingUnpaidInvoices.form.find('#use_card_profile').prop('checked', true);
                        officeAccountingUnpaidInvoices.form.find('#card_profile_select').prop('disabled', false);
                        officeAccountingUnpaidInvoices.form.find(`.form-another-card input`).prop('disabled', true);
                        officeAccountingUnpaidInvoices.form.find('#use_another_card').prop('checked', false);

                        Payment.loadCards(officeAccountingUnpaidInvoices.form.find('#card_profile_select'), officeUser.id);
                    } else {
                        officeAccountingUnpaidInvoices.form.find('#use_another_card').prop('checked', true);
                        officeAccountingUnpaidInvoices.form.find('.form-another-card input').prop('disabled', false);
                        officeAccountingUnpaidInvoices.form.find('#card_profile_select').prop('disabled', true).empty();
                        officeAccountingUnpaidInvoices.form.find('#use_card_profile').prop('checked', false);
                    }

                    officeAccountingUnpaidInvoices.form.find('[name="card_payment_amount"]').val('');
                ...
                officeAccountingUnpaidInvoices.form.find('[name="card_payment_amount"]').prop('required', true);
                officeAccountingUnpaidInvoices.form.find('[name="check_number"]').removeAttr('required');
                officeAccountingUnpaidInvoices.form.find('[name="amount"]').removeAttr('required');
            }
        });

        officeAccountingUnpaidInvoices.form.find('#card_payment_amount').on('keyup', (e) => {
            const self = $(e.target);

            if (
                officeAccountingUnpaidInvoices.officeUser.payment_method == 2
                && self.val() > 0
            ) {

                let invoiceAmount = parseFloat(self.val());

                const convenienceFeeMessageDiv = officeAccountingUnpaidInvoices.form.find('#convenienceFeeMessageDiv');
                convenienceFeeMessageDiv.removeClass('d-none');

                const convenienceFee = parseFloat(convenienceFeeMessageDiv.data('convenience-fee'));
                officeAccountingUnpaidInvoices.convenienceFeeAmount = invoiceAmount * convenienceFee / 100;
                invoiceAmount = invoiceAmount + officeAccountingUnpaidInvoices.convenienceFeeAmount;
                invoiceAmount = helper.roundToDecimal(invoiceAmount, 2);

                //officeAccountingUnpaidInvoices.form.find('[invoice-amount-due]').html(invoiceAmount);
            } /*else {
                officeAccountingUnpaidInvoices.form.find('[invoice-amount-due]')
                    .html(officeAccountingUnpaidInvoices.originalInvoiceAmount)
            }*/
        });

        officeAccountingUnpaidInvoices.form.on('submit', e => {
            //Get card info
            const cardInfo = officeAccountingUnpaidInvoices.form.find('#card_profile_select option:selected').text();
            officeAccountingUnpaidInvoices.form.find('[name="card_info"]').val(cardInfo);

            officeAccountingUnpaidInvoices.form.find('#convenience_fee_amount').val(0);

            officeAccountingUnpaidInvoices.form.find('#convenience_fee_amount').val(officeAccountingUnpaidInvoices.convenienceFeeAmount);
...
$(() => {
    officeAccountingUnpaidInvoices.init();
});

export default officeAccountingUnpaidInvoices;
