...
const OfficeRemovalOrder = {
    ...
    init() {
        ...
        $.get(helper.getSiteUrl('/get/holidays')).done(holidays => {
            OfficeRemovalOrder.holidays = holidays;
        });

        $.get(helper.getSiteUrl('/get/zone/settings')).done(settings => {
            OfficeRemovalOrder.dailyOrderCap = settings.daily_order_cap;
        })

        $.get(helper.getSiteUrl('/office/count-orders')).done(countorders => {
            OfficeRemovalOrder.countOrders = countorders;
        })
        ...
        helper.confirm("", "This action is irreversible!",
            () => {
                $.get(`/removal/order/${RemovalOrderId}/cancel`).done(res => {
                    //OfficeRemovalOrder.table.api().draw();
    ...
    loadDatatable() {
        ...
        OfficeRemovalOrder.table = $(tableId).dataTable({
            language: {
                search: "_INPUT_",
            ...
            pageLength: 10,
            dom: "rtip",
            ajax: helper.getSiteUrl("/datatable/office/orders/removal"),
            serverSide: true,
            columns: [
                {
                    data: "address",
                ...
                },
                {
                    ...
                    render(d, t, r) {
                        ...
                        if (r.order_type == 'removal') {
                            ...
                            html += '<br>';
                            if (status == OfficeRemovalOrder.status_received) {
                                html += `<span class="badge badge-pill badge-primary">Received</span>`
                            } else if (status == OfficeRemovalOrder.status_incomplete) {
                                if (r.assigned_to > 0) {
                            ...
                            } else if (status == OfficeRemovalOrder.status_scheduled) {
                                html += `<span class="badge badge-pill badge-info">Scheduled</span>`;
                            } else if (status == OfficeRemovalOrder.status_completed) {
                                html += `<span class="badge badge-pill badge-success">Completed</span>`;
                            } else if (status == OfficeRemovalOrder.status_cancelled) {
                ...
                },
                {
                    ...
                    render(d, t, r) {
                        ...
                        if (r.order_type == 'removal') {
                            ...
                            const canEdit = [
                                OfficeRemovalOrder.status_received,
                                OfficeRemovalOrder.status_incomplete
                            ];
                            ...
                            const canCancel = [
                                OfficeRemovalOrder.status_received,
                                OfficeRemovalOrder.status_scheduled,
                                OfficeRemovalOrder.status_incomplete
                            ];
                ...
                },
                {
                    ...
                    render(d, t, r) {
                        ...
                        //Display removal ordr number instead if removal order exists
                        if (r.removal) {
                            orderNumber = r.removal.order_number;
                        }
    ...
    showRemovalOrderEntries() {
        $('body').on("change", '#showRemovalOrderEntries', (event) => {
            ...
            OfficeRemovalOrder.table.api().context[0]._iDisplayLength = selected;
            OfficeRemovalOrder.table.api().draw();
        });
    },
    ...
    removalOrderSearchInput() {
        $('body').on("keyup", '#removalOrderSearchInput', (event) => {
            OfficeRemovalOrder.table.fnFilter(event.target.value);
        });
    },

    onSignPanelChange(e){
        OfficeRemovalOrder.setRemovalFee();
    },
    ...
    setPropertyInfo(order) {
        ...
        //Map
        //OfficeRemovalOrder.loadAddressOnMap(order.address, {lat: Number(order.latitude), lng: Number(order.longitude)});
    ...
    updateCalendar(savedDate) {
        ...
        $("#removalOrderDatePicker").datepicker({
            onSelect: function (dateText) {
                ...
                return OfficeRemovalOrder.updateCalendar(helper.parseUSDate(dateText));
            },
            beforeShowDay: function (date) {
                let dateString = helper.getDateStringUsa(date);

                if (OfficeRemovalOrder.holidays.includes(dateString)) {
                    return [false];
                }

                let serviceDate = $.datepicker.formatDate('yy-mm-dd', new Date(date));
                let ordersCount = OfficeRemovalOrder.countOrders[serviceDate];
                if (typeof ordersCount !== 'undefined') {
                    if (ordersCount >= OfficeRemovalOrder.dailyOrderCap) {
                        return [false];
                    }
                }
                ...
                if (OfficeRemovalOrder.zone.data) {
                    let zone = OfficeRemovalOrder.zone.data;
                    let su = zone.su;
                    let mo = zone.m;
                    let tu = zone.tu;
                    let we = zone.w;
        ...
        if (savedDate) {
            ...
            if (helper.isLastDayOfMonth(currDate) && !OfficeRemovalOrder.movedNextMonth && OfficeRemovalOrder.create) {
                ...
                OfficeRemovalOrder.movedNextMonth = true;
                setTimeout(() => {
    ...
    setDate(removalOrder) {
        ...
        if (removalOrder.service_date_type == 1) {
            OfficeRemovalOrder.updateCalendar();
            ...
            setTimeout(()=>{
                ...
                OfficeRemovalOrder.updateCalendar(helper.parseDate(removalOrder.service_date));
            }, 1000);
        }
    },
    ...
    setPostSignAccessories(order) {
        ...
        OfficeRemovalOrder.loadAccessories(order);

        if ( ! OfficeRemovalOrder.create) {
            OfficeRemovalOrder.rowCount = 0;
            if (order.removal.adjustments) {
                OfficeRemovalOrder.loadSavedAdjustments(order.removal.adjustments);
            }
    ...
    resetForm() {
        ...
        OfficeRemovalOrder.totalAdjusted = 0;
        OfficeRemovalOrder.rowCount = 0;

        OfficeRemovalOrder.setRushFee(0)
    ...
    createRemovalOrder(orderId) {
        OfficeRemovalOrder.resetForm();

        $("#loader_image").modal('show');
        $.get('/removal/get/install-order/' + orderId).done(async order => {
            OfficeRemovalOrder.create = true;
            OfficeRemovalOrder.order = order;
            OfficeRemovalOrder.order_id = order.id;

            OfficeRemovalOrder.zone = await OfficeRemovalOrder.getRemovalZone(order.id);
            OfficeRemovalOrder.settings = await helper.getZoneSettings();

            OfficeRemovalOrder.setOfficeAndAgent(order);
            OfficeRemovalOrder.setPropertyInfo(order);
            OfficeRemovalOrder.setPostSignAccessories(order);
            OfficeRemovalOrder.movedNextMonth = false;
            OfficeRemovalOrder.updateCalendar(false);
            OfficeRemovalOrder.setFooter();

            if (OfficeRemovalOrder.zone.data && OfficeRemovalOrder.settings) {
                const zone = OfficeRemovalOrder.zone.data;
                const settings = OfficeRemovalOrder.settings;
            ...
            setTimeout(async ()=>{
                ...
                const totalPosts = await OfficeRemovalOrder.countPostsAtProperty(
    ...
    editRemovalOrder(orderId) {
        ...
        $.get('/removal/get/order/' + orderId).done(async removalOrder => {
            OfficeRemovalOrder.create = false;

            //console.log(OfficeRemovalOrder.order);
            OfficeRemovalOrder.removal_order_id = removalOrder.id;

            OfficeRemovalOrder.order = removalOrder.order;
            OfficeRemovalOrder.order_id = removalOrder.order.id;

            OfficeRemovalOrder.zone = await OfficeRemovalOrder.getRemovalZone(removalOrder.order.id);
            OfficeRemovalOrder.settings = await helper.getZoneSettings();

            OfficeRemovalOrder.setOfficeAndAgent(removalOrder.order);
            OfficeRemovalOrder.setPropertyInfo(removalOrder.order);
            OfficeRemovalOrder.setPostSignAccessories(removalOrder.order);
            OfficeRemovalOrder.setDate(removalOrder, false);
            OfficeRemovalOrder.setComment(removalOrder);
            OfficeRemovalOrder.setFooter();
            OfficeRemovalOrder.setRushFee(removalOrder.rush_fee);
            ...
            if (removalOrder.adjustments) {
                OfficeRemovalOrder.loadSavedAdjustments(removalOrder.adjustments);
            }*/
    ...
    setRemovalFee() {
        ...
        const total = removalFee + OfficeRemovalOrder.getTotalFees() + parseFloat(OfficeRemovalOrder.totalAdjusted);
        $(`[removal-total]`).html(`$${total.toFixed(2)}`);

        OfficeRemovalOrder.total = total;
    },
    ...
    onSubmitForm() {
        $("#removalOrderForm").on("submit", (e) => {
            ...
            //Create/Edit action
            fd.append("create_order", OfficeRemovalOrder.create);
            //Order Id
            fd.append("order_id", OfficeRemovalOrder.order_id);
            fd.append("removal_order_id", OfficeRemovalOrder.removal_order_id);
            //Fees and total
            ...
            fd.append('total', OfficeRemovalOrder.total);
            //Button
            form.find(`[type="submit"]`).prop('disabled', true);
            form.find(`[type="submit"]`).html(`<strong class="text-white">SENDING...</strong>`);
            //Multiple posts
            fd.append("multiplePosts", OfficeRemovalOrder.multiplePosts);
            ...
            $.ajax({
                ...
                if (res.removalOrder.needPayment) {
                    ...
                    if ($('#removal_charge_source_select').length) {
                        Payment.initOfficeChargeSource({
                            sourceSelect: 'removal_charge_source_select',
                            agentSection: 'removal_agent_select_section',
                            agentSelect: 'removal_charge_agent_select',
                            ...
                            officeId: res.removalOrder.office.id,
                            officeUserId: res.removalOrder.office.user.id
                        });

                        // Default: load office's own cards
                        Payment.loadCardsFromChargeAPI($('#removal_card_profile_select'), {
                            source: 'office'
                        }, function (cards) {
                            if (cards.length > 0) {
                        ...
                        if (res.removalOrder.agent) {
                            if (res.removalOrder.agent.user.authorizenet_profile_id) {
                                ...
                                Payment.loadAgentCardsVisibleToOffice(
                                    ...
                                    res.removalOrder.agent.user.id,
    ...
    async getOthersOrdersSameProperty() {
        const getOrders = await $.get(`${helper.getSiteUrl()}/removal/orders/same/property/${OfficeRemovalOrder.order_id}`);
    ...
    processMultiplePosts() {
        $("[multiple-posts-yes-button]").on('click', async (e) => {
            OfficeRemovalOrder.multiplePosts = true;
            ...
            const orders = await OfficeRemovalOrder.getOthersOrdersSameProperty();
            if (orders) {
        ...
        $("[multiple-posts-no-button]").on('click', e => {
            OfficeRemovalOrder.multiplePosts = false;
    ...
    calculateAdjustments() {
        ...
        let charge;
        let discount;
        let totalRows = OfficeRemovalOrder.rowCount;
        ...
        OfficeRemovalOrder.totalAdjusted = totalAdjustments;

        if (OfficeRemovalOrder.totalAdjusted < 0) {
            $('[removal-adjustments]').html(`<span class="text-danger">- $${OfficeRemovalOrder.totalAdjusted*(-1)}</span>`);
        } else {
            $('[removal-adjustments]').html(`$${OfficeRemovalOrder.totalAdjusted}`);
        }
    ...
    loadSavedAdjustments(adjustments) {
        ...
        let totalAdjustments = 0;
        OfficeRemovalOrder.pricingAdjustments = {
            description: [],
            charge: [],
            discount: []
        };
        ...
        $.each(adjustments, (i, row)=> {
            OfficeRemovalOrder.rowCount++;
            let newTmpl = rowTmpl.replace(/rowCount/g, OfficeRemovalOrder.rowCount);
            rowContainer.append(newTmpl);

            $(`[name="removal_price_adjustment_description[${OfficeRemovalOrder.rowCount}]"]`).val(row.description);
            $(`[name="removal_price_adjustment_charge[${OfficeRemovalOrder.rowCount}]"]`).val(row.charge);
            $(`[name="removal_price_adjustment_discount[${OfficeRemovalOrder.rowCount}]"]`).val(row.discount);

            OfficeRemovalOrder.pricingAdjustments['description'][i] = row.description;
            OfficeRemovalOrder.pricingAdjustments['charge'][i] = row.charge;
            OfficeRemovalOrder.pricingAdjustments['discount'][i] = row.discount;
